#!/usr/bin/env python3
"""
util_liberty.py — Liberty .lib のパース基盤（ライブラリ）

ISS-00194: `util_extract_lib_csv.py` が CLI とパーサーの両方を抱えていたため、
`compare` や `make_templates` が「CSV 変換ツール」を import するという歪んだ
依存になっていた。パーサー部分を本ファイルへ切り出し、CLI と対等にする。

**本ファイルは他の util_* を一切 import しない**（標準ライブラリのみ）。

利用者:
  - util_extract_lib2csv.py   : .lib -> CSV の CLI
  - util_compare_csv.py       : COL_* 定数
  - util_make_template4json.py: parse_lib_file

公開しているもの:
  - parse_lib_file(lib_path) -> (units, scales, leakage, power, timing)
  - LibertyParser
  - COL_INDEX1 / COL_INDEX2 / COL_TIMING_VALUE / COL_POWER_VALUE / COL_LEAKAGE
  - UNITS_IN_CSV / _units_in_lib_str
"""
