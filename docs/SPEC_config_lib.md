...
## 3.3 `templates` の `index_1` / `index_2` の有効桁（2026-07-31 確定）

**有効 3 桁に丸めて記述する。**

```jsonc
// 良い
"index_1":[0.01,0.0231,0.0531,0.122,0.282,0.651,1.5]
"index_2":[0.0005,0.00134,0.00357,0.00952,0.0254,0.0679,0.181]

// 冗長（orig .lib から機械的に写した状態）
"index_1":[0.01,0.0230506,0.0531329,0.122474,0.282311,0.650743,1.5]
"index_2":[0.0005,0.00133517,0.00356533,0.00952062,0.0254232,0.0678883,0.181284]
```

### 理由

- 丸めによるズレは **0.5% 以下**。遅延・電力は slew / load の**滑らかな関数**なので、
  この程度の index のズレは結果に影響しない
- orig 比較は **NLDM 補間で index を揃える**のが本プロジェクトの前提（`charao_prj.md` の決定事項）。
  index が orig と厳密一致していなくても比較は成立する
- 下位桁は「計算で出た端数」であって物理的な意味を持たない。3 桁にすることで
  **「この桁数に意味はない」ことが読み手に伝わる**
