...
class ParamTbComb:
  ...
  logic_low_to_high_threshold_voltage:float
  logic_high_to_low_threshold_voltage:float 
  logic_threshold_low_voltage        :float
  logic_threshold_high_voltage       :float
  meas_energy                        :int
  energy_meas_low_threshold_voltage  :float
  energy_meas_high_threshold_voltage :float
  energy_start                       :float
  energy_end                         :float
...
def genFileLogic_trial1x(targetLib, targetCell, targetHarness, meas_energy, load, slope, estart, eend, spicef):
  ...
  if(os.path.isfile(spicelis)):
    ...
    model  =targetCell.model   if targetCell.model.startswith("/")   else "../"+targetCell.model
    netlist=targetCell.netlist if targetCell.netlist.startswith("/") else "../"+targetCell.netlist
    
    param = ParamTbComb(
      model       = model
      ,netlist    = netlist
      ,temp       = targetLib.temperature
      ,vdd_voltage= targetLib.vdd_voltage
      ,vss_voltage= targetLib.vss_voltage
      ,vnw_voltage= targetLib.nwell_voltage
      ,vpw_voltage= targetLib.pwell_voltage
      ,cap        = load * targetLib.capacitance_mag
      ,slew       = slope * targetLib.time_mag
      ,energy_meas_time_extent = targetLib.energy_meas_time_extent
      #,simulation_timestep     = targetCell.simulation_timestep
      ,simulation_timestep     = targetCell.simulation_timestep * targetCell.slope[0] * targetLib.time_mag
      ,logic_low_to_high_threshold_voltage=targetLib.logic_low_to_high_threshold_voltage
      ,logic_high_to_low_threshold_voltage=targetLib.logic_high_to_low_threshold_voltage
      ,logic_threshold_low_voltage        =targetLib.logic_threshold_low_voltage
      ,logic_threshold_high_voltage       =targetLib.logic_threshold_high_voltage
      ,meas_energy                        =meas_energy
      ,energy_meas_low_threshold_voltage  =targetLib.energy_meas_low_threshold_voltage
      ,energy_meas_high_threshold_voltage =targetLib.energy_meas_low_threshold_voltage
      ,energy_start                       =estart
      ,energy_end                         =eend
      ,energy_end_extent                  =eend * targetLib.energy_meas_time_extent
      ,tb_instance                        =tb_instance
      ,target_inport_val                  =targetHarness.target_inport_val
      ,target_outport_val                 =targetHarness.target_outport_val
    );
